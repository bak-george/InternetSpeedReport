    ...
    <div class="flex min-h-full items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
      <div class="w-full max-w-sm space-y-10">
        ...
        <form method="POST" class="space-y-6" action="{{route('register.store')}}">
          @csrf
          <div>
            <x-inputs.text id="name" name="name" type="name" ariaLabel="name" divClass="col-span-2" roundedDirection="rounded-t-md" placeholder="A username" />
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="text-sm/6">
              <a href="{{route('login')}}" class="font-semibold text-indigo-600 hover:text-indigo-500 ubuntu-regular">Already have an account?</a>
            </div>
          </div>
